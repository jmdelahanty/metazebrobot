...
class TestExperimentSessions:
    """Session creation, fish linking, and queries."""
    ...
    def test_create_session(self, client):
        ...
        assert resp.status_code == 201
        assert resp.json()["session_uuid"] == session_uuid

    def test_create_duplicate_session(self, client):
        session_uuid = str(uuid.uuid4())
        client.post("/sessions", json={"session_uuid": session_uuid})
        resp = client.post("/sessions", json={"session_uuid": session_uuid})
    ...
    def test_get_session_found(self, client):
        ...
        client.post(
            "/sessions",
            json={"session_uuid": session_uuid, "rig_id": "rig-02"},
        )
        resp = client.get(f"/sessions/{session_uuid}")
        assert resp.status_code == 200
        assert resp.json()["rig_id"] == "rig-02"

    def test_get_session_not_found(self, client):
        resp = client.get(f"/sessions/{uuid.uuid4()}")
        assert resp.status_code == 404

    def test_link_fish_to_session(self, client, seed_dish):
        fish_id = client.post(f"/dishes/{seed_dish}/fish", json={}).json()["fish_id"]
        session_uuid = str(uuid.uuid4())
        client.post("/sessions", json={"session_uuid": session_uuid})

        resp = client.post(
            f"/sessions/{session_uuid}/fish",
            json={"fish_id": fish_id, "dpf_at_run": 7},
        )
        assert resp.status_code == 201
        assert resp.json()["fish_id"] == fish_id

    def test_link_duplicate_fish(self, client, seed_dish):
        fish_id = client.post(f"/dishes/{seed_dish}/fish", json={}).json()["fish_id"]
        session_uuid = str(uuid.uuid4())
        client.post("/sessions", json={"session_uuid": session_uuid})
        client.post(
            f"/sessions/{session_uuid}/fish", json={"fish_id": fish_id}
        )
        # Second link should fail (UNIQUE constraint)
        resp = client.post(
            f"/sessions/{session_uuid}/fish", json={"fish_id": fish_id}
        )
        assert resp.status_code == 500

    def test_link_nonexistent_fish(self, client):
        session_uuid = str(uuid.uuid4())
        client.post("/sessions", json={"session_uuid": session_uuid})
        resp = client.post(
            f"/sessions/{session_uuid}/fish",
    ...
    def test_list_fish_in_session(self, client, seed_dish):
        fish_id = client.post(
            f"/dishes/{seed_dish}/fish", json={"subject_label": "sess-fish"}
        ).json()["fish_id"]
        session_uuid = str(uuid.uuid4())
        client.post("/sessions", json={"session_uuid": session_uuid})
        client.post(
            f"/sessions/{session_uuid}/fish",
            json={"fish_id": fish_id, "dpf_at_run": 5},
        )
        resp = client.get(f"/sessions/{session_uuid}/fish")
        assert resp.status_code == 200
        items = resp.json()["items"]
        assert len(items) == 1
        assert items[0]["fish_id"] == fish_id
        # Joined fields from fish_subjects
        assert items[0]["subject_label"] == "sess-fish"

    def test_list_sessions_for_fish(self, client, seed_dish):
        fish_id = client.post(f"/dishes/{seed_dish}/fish", json={}).json()["fish_id"]
        s1 = str(uuid.uuid4())
        s2 = str(uuid.uuid4())
        client.post(
            "/sessions",
            json={"session_uuid": s1, "run_at_utc": "2026-04-01T10:00:00"},
        )
        client.post(
            "/sessions",
            json={"session_uuid": s2, "run_at_utc": "2026-04-01T11:00:00"},
        )
        client.post(f"/sessions/{s1}/fish", json={"fish_id": fish_id})
        client.post(f"/sessions/{s2}/fish", json={"fish_id": fish_id})

        resp = client.get(f"/fish/{fish_id}/sessions")
        assert resp.status_code == 200
        items = resp.json()["items"]
        assert len(items) == 2
        # Joined session fields
        assert "rig_id" in items[0]

    def test_list_sessions_nonexistent_fish(self, client):
        resp = client.get(f"/fish/{uuid.uuid4()}/sessions")
        assert resp.status_code == 404
